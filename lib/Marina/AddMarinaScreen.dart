...
// Example function to calculate zoom level dynamically based on latitude and longitude
  ...
  void initState() {
    ...
    super.initState();
    widget.postid == null ? getLocation1() : viewposition();
    setState(() {
      isloding = true;
    });
  }
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        selectedvalue1=="Warning" ||selectedvalue1=="Anchorage"?Container():Text("Mean High Water Clearanc(Feet)",
                            style: TextStyle(
                                letterSpacing: 1,
                                color: Colors.black,
                                fontSize: 15.sp,
                        ...
                        selectedvalue1=="Marinas"|| selectedvalue1=="Warning"?Container():Column(
                          children: [
                            Container(
                              height: 50.h,
  ...
  castommapposition() async {
    if (_formKey.currentState!.validate()) {
      ...
      var meta = {
        ...
        "text_minimumapproachdepth": _minimumapproachdepth.text == null
            ? ""
            : _minimumapproachdepth.text.trim().toString(),
        "text_meanlowwaterdockdepth": _meanlowwaterdockdepth.text == null
            ? ""
        ...
        "text_maxsliplength": _MaaSlipLength.text == null
            ? ""
            : _MaaSlipLength.text.trim().toString(),
        "text_maxslipwidth": _MaaSlipwidth.text == null
            ? ""
            : _MaaSlipwidth.text.trim().toString(),
        "n1": N1,
        "n2": N2,
        "n3": N3,
        "ne1": NE1,
        ...
        if (internet) {
          authprovider()
            ...
            if (response.statusCode == 200 &&
                castompositionmarinamodal?.success == true) {
  ...
  updatecasotammarina() async {
    if (_formKey.currentState!.validate()) {
      ...
      checkInternet().then((internet) async {
        if (internet) {
          authprovider()
              .updatecastompostionapi(data, imagePaths)
              .then((response) async {
            editcasotammarinamodal =
                EditCasotamMarinaModal.fromJson(json.decode(response.body));
            if (response.statusCode == 200 &&
                editcasotammarinamodal?.success == true) {
              print("admin chalu karo bhai");
              lat = 0.0;
              long = 0.0;
        ...
        authprovider().viewsinganalpositionviewapi(data).then((response) async {
          ...
          if (response.statusCode == 200 &&
            ...
            setState(() {
              ...
              dynamic beachcyvalue =
                  viewcategorywisevieweetailmodal?.data?.metaFields?.beach;
              if (pharmacyvalue != null && pharmacyvalue is bool) {
                setState(() {
                  beach = beachcyvalue;
                });
              ...
              dynamic shopvalue =
                  viewcategorywisevieweetailmodal?.data?.metaFields?.shop;
              if (pharmacyvalue != null && pharmacyvalue is bool) {
                setState(() {
                  shop = shopvalue;
                });
              ...
              dynamic pontoonvalue =
                  viewcategorywisevieweetailmodal?.data?.metaFields?.pontoon;
              if (pharmacyvalue != null && pharmacyvalue is bool) {
                setState(() {
