  ...
  addwaring() async {
    if (_formKey.currentState!.validate()) {
      ...
      checkInternet().then((internet) async {
        if (internet) {
          authprovider().addwaringaapi(data,imagePaths).then((response) async {
            addwaringmodal =
                AddWaringmodal.fromJson(json.decode(response.body));
            if (response.statusCode == 200 && addwaringmodal?.success==true) {
              print("admin chalu karo bhai");
              EasyLoading.showSuccess(addwaringmodal?.message ?? "");
              Get.to(HomeScreen());
              ;
            } else {
